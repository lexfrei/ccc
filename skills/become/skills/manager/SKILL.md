...
- **One agent farms a whole gate.** A fix-review-fix loop (run review, address findings, re-run until clean) is a LOOP, not a fan-out. Delegate it end-to-end to a single agent that iterates to a clean verdict and reports the finished state. Do not spawn parallel reviewers and hand-assemble their verdicts — an orchestrator cannot close the feedback loop, and parallel reviewers re-derive the same context to report the same finding.
...
- **An explicit verification requirement**: "You MUST run the full test suite and report all failures" — never "make sure it works". Weak verification produces early victories: work marked passing after minimal checks.
...
- **The failure is one-way.** Outbound reports die; inbound instructions arrive. "No reply after N messages" is evidence about the return channel only — do not re-send the same instruction, it landed the first time.
