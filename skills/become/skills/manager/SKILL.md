...
- **Retire finished teammates gently.** When a teammate's scope is complete and nothing left in the queue needs its context, stand it down: request the final report and final SHA, verify by ground truth that everything is committed (git log, clean status in its worktree), then stop it. A shutdown landing mid-edit destroys uncommitted work; a done teammate left running leaks budget and attention. If you are not sure it is finished for good, tell it to stand by instead of stopping it — a kept context is cheap next to a re-brief.
