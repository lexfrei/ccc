...
- **Gate conduct** — a churn-vs-feature meta-counter with a stop flag for rounds that break previously correct text, a frozen surface scoped to what the round reads (the tree and its diff, not gitignored drafts), LGTMs burned for false claims rather than true-but-unpinned ones, section placement read as the reviewer's blocking assessment rather than truth, manager decisions bound to their premises, deletions that name what the removed fragment asserted, reviewer corrections verified like findings.
