...
(pass/fail, ok/bad, 0/1, true/false).
...
def analyze(rows, factors, outcomes, covariates, goal, target):
    ...
    if binary:
        ...
        scores = None
        if goal:
            scores = [signal_to_noise(v, goal, target) for v in per_run]
    ...
    lines.append(
        "| Factor | " + " | ".join(f"level {i}" for i in (1, 2, 3)) + " | Δ | p |"
    )
    lines.append("| --- | --- | --- | --- | --- | --- |")
    ranked = []
    for factor in factors:
        ...
        cells = [f"{lv}: {value:.3g}" for lv, value in means.items()]
        cells += [""] * (3 - len(cells))
        lines.append(
            f"| {factor} | " + " | ".join(cells) + f" | {delta:.3g} | {p_value:.3f} |"
        )
    lines.append("")
    ...
    if goal:
        ...
        flat = [factor for _, factor, _, p_value in ranked if p_value > 0.2]
        lines.append(
            "Predicted optimum: "
        ...
        if flat:
            lines.append(
                "Indistinguishable from noise (p > 0.2), so free to set by cost "
                "or convenience: " + ", ".join(flat) + "."
            )
        lines.append(
...
def main(argv):
    ...
    if goal and goal.startswith("target"):
        ...
        goal, target = "target", to_number(value) if value else None
