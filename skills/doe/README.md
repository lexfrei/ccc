...
4. **Run sheet** — the plan rendered with concrete values, two mandatory baseline runs (known-good must pass, known-bad must fail) before the array is spent, plus repetition sizing and run-order rules for flaky bugs and drifting environments. `scripts/experiment.py` keeps the whole thing in `.doe/<slug>.json` — hypothesis, column assignment, baselines, per-row results, covariates — so an array that takes two days and three sessions is resumed, not restarted. `scripts/run.py` executes the sheet when a run is a command: exit code or a `--metric` regex as the outcome, whole-array sweeps for repeats, `--randomize` against drift, every result written to the journal as it lands and each run's output kept under `.doe/<slug>-logs/`.
...
6. **Main-effects analysis** — `scripts/analyze.py` reads the finished sheet as CSV (`experiment.py csv <slug>` emits it): level means, ranking by effect size, noise floor from repeats, a permutation p per factor, the odds that a clean split is coincidence, covariate balance, and the two early exits (nothing failed / everything failed).
