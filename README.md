...
| **infostyle** | Russian text editing in информационный стиль (Ilyahov, Sarycheva). Ships **infostyle** (prose: audience and useful action first, then structure, paragraph, sentence, word and presentation, with stop-word catalogue, bureaucratese fixes and a ban on inventing the facts that replace value judgements) and **infostyle-letter** (business correspondence: informative subject lines, substance first, boundaries, the do-not-send-on-emotion check, cold emails, job replies). Rewrite or review-only mode |
