...
| **infostyle** | Russian text editing in информационный стиль (Ilyahov, Sarycheva). Ships **infostyle** (prose: context, audience and useful action first, then structure, paragraph, sentence, word and presentation, with stop-word catalogue, bureaucratese fixes, genre checklists — текст о себе, о компании, отчёты, пресс-релиз, слайды, промостраница — and a ban on inventing the facts that replace value judgements) and **infostyle-letter** (business correspondence: informative subject lines, substance first, boundaries, the do-not-send-on-emotion check, cold emails, complaints, pars, reminders, job replies). Three modes: rewrite, review-only, or re-layout without touching a single word |
| **setup-chrome-for-reddit** | Get the Playwright MCP browser past Reddit's bot detection and logged in when a task needs an account — a user-agent override that drops the `HeadlessChrome` token (enough on its own, no stealth plugins), login through the normal `old.reddit` form verified from the network log, and `browser_evaluate` selectors that read threads without enormous accessibility snapshots. Captcha solving and fingerprint evasion are out of scope |
...
| **macro-claude** | Session status hooks for the macro-claude macropad plugin — writes per-session JSON files that the Logi Plugin Service plugin reads to display live state on MX Creative Console keys |
...
| **trailer-guard** | Hook that guards commit trailers on the current branch: after a command that writes, moves or publishes commits it flags your own commits that lost `Signed-off-by`, carry a `Claude-Session` line, or name a model in `Assisted-by` instead of the neutral `Assisted-by: LLM`, and it blocks `git push`, `git send-email`, `git send-pack`, `git imap-send`, `git subtree push`, `git svn dcommit/set-tree`, `git p4 submit`, `gh pr create/ready/merge` and `gh stack submit/sync/push/merge/link` until they are fixed; a branch stacked on another local branch is judged and repaired from that branch's tip, and the repair recipe rewrites only your commits |
