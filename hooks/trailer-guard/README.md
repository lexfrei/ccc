...
Guards the trailers of your own commits on the current branch before CI or a reviewer does. It catches three things: a commit that lost its `Signed-off-by`, a commit carrying a `Claude-Session:` line, and an `Assisted-by:` trailer that names a model instead of the neutral `Assisted-by: LLM`. It reports them after a command that writes, moves or publishes commits, and blocks the commands that would publish the branch while they remain.
...
One script, two hook events. As a `PostToolUse` hook it runs after a `Bash` tool call that can change what the guard would say — `git commit`, `merge`, `rebase`, `cherry-pick`, `revert`, `am`, `reset`, `checkout`, `switch`, `pull`, `filter-branch`, `gh stack rebase`, and everything the `PreToolUse` arm blocks — inspects the commits the current branch added over its base, and reports defects so the model sees them at once. A read is not one of them: `git log`, `git status` and `git diff` cannot change the answer, and firing after them repeats a standing report once per command until the commit is repaired, which buries it. The cost is that a rewrite by some other route — a script, an editor-driven rebase — is not noticed until the next command that is on the list; nothing defective reaches a remote regardless, because the `PreToolUse` arm is unchanged and blocks on the commits themselves. As a `PreToolUse` hook it runs before `git push`, `git send-email`, `git send-pack`, `git imap-send`, `git subtree push`, `git svn dcommit`, `git svn set-tree`, `git p4 submit`, `gh pr create`, `gh pr ready`, `gh pr merge`, `gh stack submit`, `gh stack sync`, `gh stack push`, `gh stack merge` and `gh stack link`, and denies the command while defects remain. Amend, rebase, status, log and `gh stack rebase` are never blocked, so the guard never stands between you and the fix. `git format-patch` and `git request-pull` are not blocked either: they write to a file or to stdout and send nothing anywhere, and neither is `git lfs push`, which uploads blobs rather than commits, so no trailer travels with it. `git send-email --dry-run` is blocked along with the rest, the same way `git push --dry-run` is. What the line runs is what counts, not what it mentions. The text zones come out first, a heredoc body and any quoted argument of more than one word, though a double-quoted one carrying a `$(` or a backtick stays in, since that really would run. In what is left the verb has to sit where a git subcommand sits. So a commit message naming a guarded command is not that command, as long as a quoted message carries no shell separator (`;`, `|`, `&`, a newline), since the line is cut on those before quotes are read and a fragment holding the verb is then judged as code. A heredoc body has no such limit, it is set aside whole, so that is the way to write a message with separators in it. A value handed to an option (`git log --grep push`) is not a command either. A text zone is read as a command again where its segment runs a shell, so `bash -c 'git push'`, `eval "git push"` and `bash <<EOF` are still blocked. Both events fire on the result rather than on the shape of the command, which covers every rewrite mechanism, including ones not invented yet.
...
Commands that publish a whole stack (`gh stack submit`, `sync`, `push`, `merge`, `link`) push every branch in it, so before them the guard judges every layer beneath HEAD. A `git push`, and any `gh pr` command that makes one, carries the whole ancestry too, so it judges a lower layer for as long as that layer is on no remote; once the layer is pushed it belongs to its own branch's guard and a defect there no longer holds this branch. Defects in this branch get the usual recipe. Defects in a lower layer are listed under the branch that owns them, with no recipe: switch to that branch, where the guard prints the recipe for exactly those commits, then rebase the branches above it. Layers above HEAD are invisible to the guard, which reads the first-parent line and so only ever looks downwards; `gh stack submit` run from a middle layer publishes the layers above it unjudged, so run stack commands from the top layer.
